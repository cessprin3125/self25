...
/////////////////////////////////
  ...
  Self.on("ready", () => {
    ...
    console.log(chalk.cyan.bold("Selfbot ") + chalk.yellow.bold("By") + chalk.green.bold(" Julia"));
    console.log(chalk.black('a'));
    console.log(chalk.magenta("///////////") + chalk.blue("///////////") + chalk.cyan("///////////"));
    console.log(chalk.bgMagenta('//   S E L F '));
    console.log(chalk.yellow("///////////") + chalk.red("///////////") + chalk.green("///////////"));
    console.log(chalk.bgCyan('//           B Y '));
    console.log(chalk.red('///////////') + chalk.white('///////////') + chalk.cyan('///////////'));
    console.log(chalk.bgRed.bold('//               J U L I A A A'));
    console.log(chalk.green('///////////') + chalk.gray('///////////') + chalk.yellow('///////////'));
...
Self.on('message', async message => {
  ...
  var self = "Selfbot By Julia ‖ Version 2.0 "
...
if(cmd === (prefix + 'play')){
    ...
    message.channel.send("Activité définie en PLAYING : " + txt)
    .then(function (message) { 
        message.delete(5000);
    });
...
if(cmd === (prefix + 'stream')){
    ...
    Self.user.setActivity(txt, {type : "STREAMING", url : "https://twitch.tv/something"})
    message.channel.send("Activité définie en STREAMING : " + txt)
    .then(function (message) { 
        message.delete(5000);
    });
...
if(cmd === (prefix + 'listen')){
    ...
    message.channel.send("Activité définie en LISTENING : " + txt)
    .then(function (message) { 
        message.delete(5000);
    });
...
if(cmd === (prefix + 'watch')){
    ...
    message.channel.send("Activité définie en WATCHING : " + txt)
    .then(function (message) { 
        message.delete(5000);
    });
...
if(cmd === (prefix + 'on')){
    ...
    message.channel.send("Vous êtes désormais **En ligne**. ✅")
    .then(function (message) { 
        message.delete(5000);
    });
...
if(cmd === (prefix + 'off')){
    ...
    message.channel.send("Vous êtes désormais en mode **Invisible**. ✅")
    .then(function (message) { 
        message.delete(5000);
    });
    console.log(chalk.green.bold('Statut mis à jour.'));
...
if(cmd === (prefix + 'inac')){
    ...
    message.channel.send("Vous êtes désormais en mode **Inactif**. ✅")
    .then(function (message) { 
        message.delete(5000);
    });
    console.log(chalk.green.bold('Statut mis à jour.'));
...
if(cmd === (prefix + 'npd')){
    ...
    message.channel.send("Vous êtes désormais en mode **Ne pas déranger**. ✅")
    .then(function (message) { 
        message.delete(5000);
    });
    console.log(chalk.green.bold('Statut mis à jour.'));
...
if(cmd === (prefix + 'say')){
    message.delete()
    message.channel.send("Le selfbot est bien en marche ! :)")
    .then(function (message) { 
        message.delete(5000);
    });
...
if(message.content.startsWith(prefix + 'calc')) { 
  ...
  let answer;
    ...
    .addField("**Equation:**", question)
  	...
  	.addField("**Résultat:**", `${answer === undefined ? 'Calcul impossible.' : `${answer}`}`)
   message.channel.send(calcule).then(m => m.delete(60500)).catch(console.error)
...
if(cmd === (prefix + 'pp')){
  message.delete()
   ...
   if(attachment){
    ...
    Self.user.setAvatar(avatar).catch(() => message.channel.send("Une erreur est survenue.").then(function(){ message.delete(5000)}))
...
if(cmd === (prefix + 'pdp')){
    ...
    if(!m){
        message.channel.send("Vous n'avez pas mentionné d'utilisateur ! ❌")
        .then(function (message) { 
            message.delete(5000);
        });
    } else {
      if(m.avatar === null) return;
        Self.user.setAvatar(m.avatarURL)
        message.channel.send("Votre avatar a été mis à jour ! ✅").catch(() => message.channel.send("Une erreur est survenue.").then(function(){ message.delete(5000)}))
        .then(function (message) { 
            message.delete(5000);
        });
...
if(cmd === (prefix + 'avatar')){
    ...
    if(m) {
        ...
        avatar.addField("Pour avoir le lien de l'avatar,", `[Cliquez ici](${m.avatarURL})`)
...
if(cmd === (prefix + 'leet')){
  ...
  if (!args[0]) { return message.edit(oublie) }
...
//ASCII ->
  ...
  //INVERSE
    ...
    if (cmd === prefix + "inverse") {
      ...
      if(!args[1]) {
        ...
        message.edit(args.slice(1).join(' ').split('').reverse().join(''));
    ...
    if (cmd === prefix + "espace") {
      ...
      if(!args[1]) {
        ...
        message.edit(args.slice(1).join(' '.repeat(amount / 2)).split('').join(' '.repeat(amount)));
  ...
  if (args[0].toLocaleLowerCase() === prefix + '8b'){
    ...
    var rep = ["Oui", "Je ne sais pas", "Non", "Peut être ", "Bonne question....", "Redemandez moi plus tard."];
...
if(cmd === prefix + 'si'){
    ...
    .addField('**__Date de création__** : ', message.guild.createdAt.toDateString(), true)
    ...
    .addField('Pour voir l\'icône du serveur,', `[Cliquez ici](${message.guild.iconURL})`)
    .setFooter(self)
 message.channel.send(embed)
 .then(function (message) { 
    message.delete(120100);
...
if(cmd === (prefix + "ui")){
    ...
    if(message.guild) { 
      ...
      if (member) {
          ...
          embed.addField("**__Nickname__** : ", `${member.nickname !== null ? ` Nickname: ${member.nickname}` : " Aucun. "}`, true)
          embed.addField("**__Statut__** :", `${member.user.presence.status}`, true)
          embed.addField("**__Activité en cours__** :", `${member.user.presence.game ? `${member.user.presence.game.name}` : "Aucune activité."}`, true)
          embed.addField("**__Rôles__** :", `${member.roles.filter(r => r.id !== message.guild.id).map(roles => `\`${roles.name}\``).join(" **|** ") || "Aucun rôle. "}`, true)
          embed.addField("__**Compte créé le**__ : ", member.user.createdAt.toDateString())
          embed.addField("Pour voir l'avatar, cliquez ici : ", `[Avatar](${member.user.avatarURL})`)
       ...
       console.log(chalk.magenta('Infos utilisateurs envoyées !'));
          ...
          embed.addField("__**Nickname**__ :", `${message.author.nickname !== null ? ` Nickname : ${message.author.nickname}` : " Aucun"}`, true)
          embed.addField("__**Statut**__ :", `${message.author.presence.status}`, true)
          embed.addField("__**Activité en cours**__  :", `${message.author.presence.game ? `${message.author.presence.game.name}` : "Aucune activité."}`, true)
          embed.addField("__**Compte créé le**__ : ", message.author.createdAt.toDateString())
          embed.addField("Pour voir l'avatar, cliquez ici : ", `[Avatar](${message.author.avatarURL})`)
        ...
        if (!member1) {
          ...
          embed.addField("**__Statut__** :", `${message.author.presence.status}`, true)
          embed.addField("__**Activité en cours**__ :", `${message.author.presence.game ? ` ${message.author.presence.game.name}` : "Aucune activité."}`, true)
          embed.addField("__**Compte créé le**__ : ", message.author.createdAt.toDateString())
          embed.addField("Pour voir l'avatar, cliquez ici : ", `[Avatar](${message.author.avatarURL})`)
          embed.setFooter(`Informations de ${message.author.username}`)
          ...
          embed.addField("**__Activité en cours__** : ", `${member1.presence.game ? ` ${member1.presence.game.name}` : "Aucune activité."}`, true)
          embed.addField("**__Compte créé le__** : ", member1.createdAt.toDateString())
          embed.addField("Pour voir l'avatar, cliquez ici : ", `[Avatar](${member1.avatarURL})`)
  ...
  if(cmd === (prefix + "clear")){ 
    ...
    message.channel.fetchMessages({ limit: Math.min(messagecount + 1, 100, 200, 300, 400, 500, 600, 700, 800, 999) })
        .then(messages => {
            messages.forEach(m => { m.delete().catch(console.error); deletedMessages++; });
        }).then(() => {
            if (deletedMessages === -1) deletedMessages = 0;
            message.channel.send(":white_check_mark: " + messagecount + " message(s) ont été suprimés.").then(message => message.delete(3500)).then(function (message) { 
                message.delete(5000);
              });
        }).catch(console.error);
...
if(cmd === (prefix + "hug")){
    ...
    let hug = ["https://cdn.weeb.sh/images/rJ_slRYFZ.gif","https://cdn.weeb.sh/images/Sk2gmRZZG.gif","https://cdn.weeb.sh/images/SyaAd_7vW.gif"]
  
    let embed = new Discord.RichEmbed()
            embed.setDescription("💖 " + message.author.username + ' fait un câlin à ' + member)
  ...
  if(cmd === (prefix + "kiss")){
    ...
    let member = message.mentions.users.first()
    let kiss = ["https://cdn.weeb.sh/images/ryoW3T_vW.gif","https://cdn.weeb.sh/images/SJ8I2Tuv-.gif","https://cdn.weeb.sh/images/B12g3TOPZ.gif","https://cdn.weeb.sh/images/rkFSiEedf.gif"]
  
    let ks = new Discord.RichEmbed()
           ks.setDescription("💞 " + message.author.username + ' fait un bisous à ' + member)
  ...
  if(cmd === (prefix + "cry")){
    ...
    let cry = ["https://cdn.weeb.sh/images/rJUbkgqyf.gif","https://cdn.weeb.sh/images/Hy4QmU7PZ.gif","https://cdn.weeb.sh/images/rJUujgJ5Z.gif"]
  ...
  if(cmd === (prefix + "punch")){
    ...
    var m = message.mentions.users.first()
    let punch = ["https://cdn.weeb.sh/images/HJfGPTWbf.gif", "https://cdn.weeb.sh/images/SkFLH129z.gif",]
    let embed = new Discord.RichEmbed()
    embed.setDescription( "👊🏽" + message.author.username + ' donne un coup de poing à ' + m)
  ...
  if(cmd === (prefix + 'pub')){
      ...
      let lien = args[2]
      pub.setAuthor(nom)
     pub.setThumbnail(message.author.avatarURL)
      pub.addField("Hésitez pas à rejoindre mon serveur !", `[Cliquez ici :)](${lien})`)
      pub.setColor("7401DF")
...
if(cmd === (prefix + 'sondage')){
    ...
    if(!question) return message.channel.send("Vous avez oublié la question. :x:")
...
if(cmd === (prefix + 'pfc')){
    ...
    if(message.content.startsWith(prefix + 'pfc')){
      message.channel.send("Veuillez choisir un élément entre **pierre**, **feuille** et **ciseaux**.").then(function (message){
        message.delete(5000)
      })
    }
...
if(cmd === (prefix + 'creater')){
    ...
    message.channel.send("Le rôle " + "**" + name + "**" + " a été créé.")
    .then(function (message) { 
...
if(cmd === (prefix + 'ban')){
    ...
    if(!member) return message.channel.send(oublie).then(function (message) { 
        message.delete(5000);
      });
    if(reason) {
        message.guild.ban(member)
        message.channel.send(member + " a été banni(e) pour la raison suivante : " + "**" + reason + "**")
        .then(function (message) { 
            message.delete(5000);
          });
        ...
        message.channel.send(member + " a été banni(e).")
        .then(function (message) { 
            message.delete(5000);
          });
          console.log('Le membre a été banni.');
...
if(cmd === (prefix + 'kick')){
    ...
    let member = message.mentions.members.first() || message.guild.members.get(args[1])
    let reason = args.slice(2).join(" ")
    if(!member) return message.channel.send(oublie)
    .then(function (message) { 
        message.delete(5000);
      });
    if(reason) {
        member.kick()
        message.channel.send(member + " a été kick pour la raison suivante : " + "**" + reason + "**" )
        .then(function (message) { 
            message.delete(60500);
          });
...
if (cmd === (prefix + 'createc')) {
 ...
 console.log(chalk.yellow('Les channels ont été créés.'));
  ...
  //CREER MASSE ROLES
  if (cmd === (prefix + "rolec")) {
    var color = ["FF0000", "FF4000", "2E64FE", "6A0888", "AC58FA", "58FAF4", "FF0040"]
  ...
  if(cmd === (prefix + 'hack')){
    message.delete()
    var m = message.mentions.users.first()
    message.channel.send("```Veuillez patienter, nous récupérons quelques données... 💤```")
    .then(message => {
  setTimeout(() => {
        message.edit('░░░░░░░░░░ 0%');
      }, 1000);
  ...
  if(cmd === (prefix + "rename")){
    ...
    console.log(chalk.bgBlue("L'icône et le nom ont été modifiés avec succès."))
  }
  ...
  if (month < 1 || month > 12) {
      return message.reply('Veuillez entrer un mois correct ( entre 1 et 12 ).');
  }
  ...
  embed.addField("Pour en savoir plus sur ton signe astrologique..",`[Clique ici...](https://www.elle.fr/Astro/Horoscope/Caractere/Capricorne)`)
  embed.addField("Pour consulter votre horoscope,",`[Cliquez ici...](https://www.elle.fr/Astro/Horoscope/Quotidien/Capricorne)`)
  embed.setFooter(self)
  ...
  if (day >= 20 && day <= 31) {
    ...
    embed.addField("Pour en savoir plus sur ton signe astrologique..",`[Clique ici...](https://www.elle.fr/Astro/Horoscope/Caractere/Verseau)`)
    embed.addField("Pour consulter votre horoscope,",`[Cliquez ici...](https://www.elle.fr/Astro/Horoscope/Quotidien/Verseau)`)
    embed.setFooter(self)
    embed.setColor("FF0000")
    message.channel.send(embed)
    };
  ...
  if (day >= 19 && day <= 29) {
    ...
    embed.addField("Pour en savoir plus sur ton signe astrologique..",`[Clique ici...](https://www.elle.fr/Astro/Horoscope/Caractere/Poisson)`)
    embed.addField("Pour consulter votre horoscope,",`[Cliquez ici...](https://www.elle.fr/Astro/Horoscope/Quotidien/Poisson)`)
    embed.setFooter(self)
    embed.setColor("FF0000")
    message.channel.send(embed)
    };
  ...
  if (day >= 21 && day <= 31) {
    ...
    embed.addField("Pour en savoir plus sur ton signe astrologique..",`[Clique ici...](https://www.elle.fr/Astro/Horoscope/Caractere/Belier)`)
    embed.addField("Pour consulter votre horoscope,",`[Cliquez ici...](https://www.elle.fr/Astro/Horoscope/Quotidien/Belier)`)
    embed.setFooter(self)
    embed.setColor("FF0000")
    message.channel.send(embed)
    }; 
  ...
  if (day >= 20 && day <= 31) {
    ...
    embed.addField("Pour en savoir plus sur ton signe astrologique..",`[Clique ici...](https://www.elle.fr/Astro/Horoscope/Caractere/Taureau)`)
    embed.addField("Pour consulter votre horoscope,",`[Cliquez ici...](https://www.elle.fr/Astro/Horoscope/Quotidien/Taureau)`)
    embed.setFooter(self)
    embed.setColor("FF0000")
    message.channel.send(embed)
    }; 
  ...
  if (day >= 21 && day <= 31) {
    ...
    embed.addField("Pour en savoir plus sur ton signe astrologique..",`[Clique ici...](https://www.elle.fr/Astro/Horoscope/Caractere/Gemeaux)`)
    embed.addField("Pour consulter votre horoscope,",`[Cliquez ici...](https://www.elle.fr/Astro/Horoscope/Quotidien/Gemeaux)`)
    embed.setFooter(self)
    embed.setColor("FF0000")
    message.channel.send(embed)
    };
  ...
  if (day >= 21 && day <= 31) {
    ...
    embed.addField("Pour en savoir plus sur ton signe astrologique..",`[Clique ici...](https://www.elle.fr/Astro/Horoscope/Caractere/Cancer)`)
    embed.addField("Pour consulter votre horoscope,",`[Cliquez ici...](https://www.elle.fr/Astro/Horoscope/Quotidien/Cancer)`)
    embed.setFooter(self)
    embed.setColor("FF0000")
    message.channel.send(embed)
    };
  ...
  if (day >= 23 && day <= 31) {
    ...
    embed.addField("Pour en savoir plus sur ton signe astrologique..",`[Clique ici...](https://www.elle.fr/Astro/Horoscope/Caractere/Lion)`)
    embed.addField("Pour consulter votre horoscope,",`[Cliquez ici...](https://www.elle.fr/Astro/Horoscope/Quotidien/Lion)`)
    embed.setFooter(self)
    embed.setColor("FF0000")
    message.channel.send(embed)
    };
  ...
  if (day >= 23 && day <= 31) {
    ...
    embed.addField("Pour en savoir plus sur ton signe astrologique..",`[Clique ici...](https://www.elle.fr/Astro/Horoscope/Caractere/Vierge)`)
    embed.addField("Pour consulter votre horoscope,",`[Cliquez ici...](https://www.elle.fr/Astro/Horoscope/Quotidien/Vierge)`)
    embed.setFooter(self)
    embed.setColor("FF0000")
    message.channel.send(embed)
    };
  ...
  if (day >= 23 && day <= 31) {
    ...
    embed.addField("Pour en savoir plus sur ton signe astrologique..",`[Clique ici...](https://www.elle.fr/Astro/Horoscope/Caractere/Balance)`)
    embed.addField("Pour consulter votre horoscope,",`[Cliquez ici...](https://www.elle.fr/Astro/Horoscope/Quotidien/Balance)`)
    embed.setFooter(self)
    embed.setColor("FF0000")
    message.channel.send(embed)
    }; 
  ...
  if (day >= 23 && day <= 31) {
    ...
    embed.addField("Pour en savoir plus sur ton signe astrologique..",`[Clique ici...](https://www.elle.fr/Astro/Horoscope/Caractere/Scorpion)`)
    embed.addField("Pour consulter votre horoscope,",`[Cliquez ici...](https://www.elle.fr/Astro/Horoscope/Quotidien/Scorpion)`)
    embed.setFooter(self)
    embed.setColor("FF0000")
    message.channel.send(embed)
    };
  ...
  if (day >= 22 && day <= 31) {
    ...
    embed.addField("Pour en savoir plus sur ton signe astrologique..",`[Clique ici...](https://www.elle.fr/Astro/Horoscope/Caractere/Sagittaire)`)
    embed.addField("Pour consulter votre horoscope,",`[Cliquez ici...](https://www.elle.fr/Astro/Horoscope/Quotidien/Sagittaire)`)
    embed.setFooter(self)
    embed.setColor("FF0000")
    message.channel.send(embed)
    };
  ...
  if(cmd === (prefix + 'stats')){
    ...
    var inactif = message.guild.members.filter(m => m.presence.status === 'idle').size
    var npd = message.guild.members.filter(m => m.presence.status === 'dnd').size
    var offline = message.guild.members.filter(m => m.presence.status === 'offline').size
 ...
 if(cmd === (prefix + 'whoisip')){
     ...
     if(!ip) return message.channel.send("Veuillez donner une IP à localiser !")
     let whois = new Discord.RichEmbed()
     whois.setTitle("🐱‍💻 __**Localisation de l'IP**__ : ")
     whois.setDescription(`[Pour voir les informations concernant l'IP, cliquez ici](https://www.ip-tracker.org/locator/ip-lookup.php?ip=${ip})`)
...
if(cmd === (prefix + 'lien')){
    ...
    lien.setDescription(`[Ne clique surtout pas ici !!](https://disçordapp.com/D3HC37)`)
...
if(cmd === (prefix + 'wiki')){
  ...
  wiki.setDescription(`[Cliquez ici pour voir les résultats de votre recherche ! ](https://fr.wikipedia.org/wiki/${search})`)
  wiki.setColor("")
  wiki.setFooter(self)
...
if (message.content.startsWith(prefix + "multistream")) {
    ...
    if (rotate === 0)
     Self.user.setActivity(Self.Self.text1, {type: "STREAMING", url: "https://www.twitch.tv/tsm_viss" })
     rotate = 1;
    if (rotate === 1) {
      Self.user.setActivity(Self.Self.text2, {type: "STREAMING", url: "https://www.twitch.tv/Darliing" })
     rotate = 2;
    if (rotate === 2)
      Self.user.setActivity(Self.Self.text3, {type: "STREAMING", url: "https://www.twitch.tv/Julia" })
     rotate = 3;
    if (rotate === 3)
    Self.user.setActivity(Self.Self.text4, {type: "STREAMING", url: "https://www.twitch.tv/something" })
     rotate
    }
    },5 * 1000 );
message.channel.send("Le multistream a été activé.").then(function(message){
  message.delete(5000)
})
...
if(cmd === (prefix + 'mot')){
  ...
  result.setDescription(`[Cliquez ici pour voir la définition du mot !](https://www.le-dictionnaire.com/definition/${mot})`)
  ...
  if(cmd === (prefix + 'nickname')){
    ...
    if(!text) return message.channel.send("Vous avez oublié des informations.") ;
    if(!message.guild) return message.channel.send('Cette commande n\'est exécutable que sur un serveur ! ') ;

    message.guild.members.get(Self.user.id).setNickname(text)
    message.channel.send("Pseudo re - défini en : **" + text + "**.").then(function (message){
      message.delete(5000)
    });
...
message.channel.send("Le multinickname a été activé.").then(function(message){
  message.delete(5000)
})
...
if(cmd === (prefix + 'help')){
  message.delete()
   let help = new Discord.RichEmbed()
    ...
    help.addField("**__𝗦𝗲𝘁𝘂𝗽 🔨__** : ","Pour afficher les différents commandes setup.")
    help.addField("**__𝗧𝗼𝗼𝗹𝘀 ⚙__** : ", " Pour afficher les commandes utilitaires.")
    help.addField("**__𝗙𝘂𝗻 🎉__** : "," Pour afficher les différentes commandes fun disponibles.")
    help.addField("**__𝗥𝗮𝗶𝗱 ☢__** : ", " Pour afficher les différentes commandes raid disponibles.")
    help.addField("**__𝗣𝗿𝗲𝗺𝗶𝘂𝗺 ⚜__** :", " Afficher les commandes premium disponibles.")
...
if(cmd === (prefix + 'setup')){
  ...
  setup.addField("🔨 **play `text`**: ", " Définir votre statut en : `Joue à **text**`")
  setup.addField("🔨 **stream `text`** : ", " Définir votre statut en : `Streame **text**`")
  setup.addField("🔨 **listen `text`** : ", " Définir votre statut en `Ecoute **text**`")
  setup.addField("🔨 **watch `text`** : ", " Définir votre statut en `Regarde **text**`")
  setup.addField("🔨 **null** : ", "Pour reset son activité.")
  setup.addField("🔧 **on** : ", " Votre statut sera `En ligne`")
  setup.addField("🔧 **off ** : ", " Votre statut sera défini en `Hors - ligne`")
  setup.addField("🔧 **npd** : ", " Votre statut sera défini en `Ne pas déranger`")
...
if(cmd === (prefix + 'tools')){
  ...
  let tools = new Discord.RichEmbed()
    tools.setAuthor("Voici les différentes commandes utilitaires disponibles :")
    tools.addField("🔎 **ui @user :** ", " Afficher ses informations ou celles de l'utilisateur mentionné.")
    tools.addField("🔍 **si** : ", " Afficher les informations du serveur où la commande est effectuée.")
    tools.addField("📸**avatar @user** : ", " Afficher sa photo de profil ou celle de l'utilisateur mentionné.")
    tools.addField("🆑 **del** : ", "Supprimer tous ses messages.")
    tools.addField("🗑 **clear `x`** :", " Supprimer un nombre défini de messages.")
    tools.addField("📡 **ping** :", " Afficher le ping du bot.")
    tools.addField("🌙 **say** : ", " Pour vérifier si le self est bien en marche.")
    tools.addField("🔄 **restart** : "," Re - démarrer le selfbot. ")
   ...
   if(cmd === (prefix + 'fun')){
    ...
    fun.addField("🤷‍ **8b `question`** : ", " Poser une question à laquelle sera donnée une réponse aléatoire.")
    fun.addField("🖋 **inverse `text`** : ", " Inverser le texte qui précédera la commande.")
    fun.addField("🖋 **espace `text` ** : ", " Le texte qui précédera la commande sera espacé.")
    fun.addField("🖋 **ascii `text`** : ", " Votre texte sera envoyé sous forme de ascii.")
    fun.addField("✍ **embed [color] [text] ** : ", "Envoyer sous forme de embed le message de votre choix.")
    fun.addField("✍ **image [color] [image en attachement] [description] :** ", " Envoyer sous forme de embed l'image de votre choix.")
    fun.addField("💞 **hug @user** : ", " Faire un câlin à l'utilisateur mentionné.")
    fun.addField("💏 **kiss @user** : ", " Faire un bisous à l'utilisateur mentionné.")
    fun.addField("👊 **punch @user** : ", " Donner un coup de poing avec l'utilisateur mentionné.")
    fun.addField(":frowning: **cry** : ", " Pleurer.")
    fun.addField("🆗 **ok** : ", " Envoyer un gif **okay**.")
    fun.addField("♏ **zodiac [mois] [jour]** : ", " Cette commande vous donnera votre signe du zodiaque. ( Version standarde )")
    fun.addField("⚠ **hack @user** : ", " Récupérer le token d'une personne ( troll.. ).")
    fun.addField(":question: **sondage [question]** : ", " Organiser un sondage.")
    fun.addField(":interrobang: *lsv [info]** : ", " Organiser un « Le saviez vous ? ».")
    fun.addField("✊🏽 **pfc [pierre / feuille / ciseaux ]** : ", " Vous choisissez un des éléments entre crochets auquel le bot répondra aléatoirement ( pierre feuille ciseaux ).")
    fun.addField("🐱‍💻 **whoisip [Ip]** : ", "Cette commande effectuera des recherches sur l'IP que vous aurez saisie.")
    fun.addField("🔎 **sgif [mot clé]** : "," Rechercher un gif.")
    ...
    if(cmd === (prefix + 'raid')){
      ...
      raid.addField("❌ **deletec** :", " Cette commande supprimera tous les channels et créera un channel nommé Général.")
      raid.addField("❌ **createc [nombre] [nom]**", " Créer plusieurs channels textuels avec le nom de votre choix.")
      raid.addField("❌ **createv [nombre] [nom]** :", " Créer plusieurs channels vocaux avec le nom de votre choix.")
      raid.addField("❌ **rolec [nom]** : ", " Plusieurs rôles seront créés avec le nom de votre choix.")
      raid.addField("❌ **d-role** : ", " Supprimer tous les rôles. ")
      raid.addField("❌ **kall** : "," Expulser tous les membres. ")
      raid.addField("❌ **ball** : "," Bannir tous les utilisateurs. ")
      raid.addField("❌ **spam [nombre] [texte]** :", " Le message de votre choix sera spammé.")
      raid.addField("❌ **rename [nouveau nom] avec l'image en attachement.** :", " Cette commande changera la pp et le nom du serveur par ce que vous aurez défini.")
      raid.setFooter(self)
      raid.setColor("RED")
    ...
    premium.addField("⚜ **pp [image en attachement] // [ lien de l'image]**: ", "Votre pp sera remplacée par le lien de l'image donnée.")
    premium.addField("⚜ **pdp @user**", " Votre photo de profil sera remplacée par celle de l'utilisateur mentionné.")
    premium.addField("⚜ **name `text`** :"," Votre pseudo sera remplacé par celui de votre choix." )
    premium.addField("⚜ **nickname `text`** : ", "Cette commande changera votre pseudo par celui de votre choix sur le serveur où cette dernière sera effectué.")
    premium.addField("⚜ **get [nom d'un emoji]** : ", " Récupérer un emoji ( le lien qui vous permettra de le télécharger ensuite).")
    premium.addField("⚜ **pub [nom de votre serveur] [lien du serveur]** :", " Cela enverra votre pub sous forme de embed. ( By pass les anti pub )")
    premium.addField("⚜ **zodiac [mois] [jour]** : "," Cette commande vous donnera votre signe du zodiaque, un article dessus & votre horoscope du **jour** ! ( Commande complète uniquement pour la version premium)")
    premium.addField("⚜ **multistream** : ", "Activer son multistream.")
    premium.addField("⚜ **multinick** : ", "Activer le multinickame sur le serveur où la commande a été effectuée.")
    ...
    premium.addField("⚜ **emote [nom] [emoji en attachement]","Ajouter un emoji")
